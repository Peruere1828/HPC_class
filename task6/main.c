...
int main(int argc, char *argv[]) {
  ...
  int i, j, left, right, above, below, iter = 0, itmax = 80000;
  ...
  u = (double(*)[nly + 1]) calloc((nlx + 1) * (nly + 1), sizeof(double));
  ut = (double(*)[nly + 1]) calloc((nlx + 1) * (nly + 1), sizeof(double));
  f = (double(*)[nly])calloc(nlx * nly, sizeof(double));
  ...
  if (myidx < (nx - 1) % npx)
    x0 = myidx * (nx - 1) / npx + myidx;
  else
    x0 = myidx * (nx - 1) / npx + (nx - 1) % npx;
  if (myidy < (ny - 1) % npy)
    y0 = myidy * (ny - 1) / npy + myidy;
  else
    y0 = myidy * (ny - 1) / npy + (ny - 1) % npy;
  ...
  for (iter = 1; iter <= itmax; iter++) {
    ...
    for (i = 1; i < nlx; i++)
      for (j = 1; j < nly; j++)
        if ((int)(x0 + i + y0 + j) % 2 == 0)
          u[i][j] = f[i][j] + dx * (u[i + 1][j] + u[i - 1][j]) +
                    dy * (u[i][j + 1] + u[i][j - 1]);

    MPI_Sendrecv(&u[1][1], 1, type1, below, 111, &u[1][0], 1, type1, below, 111,
                 MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&u[1][nly - 1], 1, type1, above, 111, &u[1][nly], 1, type1,
                 above, 111, MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&u[1][1], 1, type2, left, 111, &u[0][1], 1, type2, left, 111,
                 MPI_COMM_WORLD, &status);
    MPI_Sendrecv(&u[nlx - 1][1], 1, type2, right, 111, &u[nlx][1], 1, type2,
                 right, 111, MPI_COMM_WORLD, &status);

    // BLACK
    for (i = 1; i < nlx; i++)
      for (j = 1; j < nly; j++)
        if ((int)(x0 + i + y0 + j) % 2 == 1)
          u[i][j] = f[i][j] + dx * (u[i + 1][j] + u[i - 1][j]) +
                    dy * (u[i][j + 1] + u[i][j - 1]);
    ...
    for (i = 1; i < nlx; i++)
      for (j = 1; j < nly; j++) {
        rtmp = f[i][j] - u[i][j] + dx * (u[i + 1][j] + u[i - 1][j]) +
               dy * (u[i][j + 1] + u[i][j - 1]);
        if (res < fabs(rtmp))
          res = fabs(rtmp);
      }
