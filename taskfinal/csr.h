...
struct ELL {
  ...
  ~ELL() { free(val); free(col); }
  ELL(const ELL &) = delete;
  ELL &operator=(const ELL &) = delete;
};
...
ELL csr_to_ell(const CSR &A) {
  ...
  size_t total = (size_t)A.n * max_len;
  E.val = alloc_float(total);
  E.col = alloc_int(total);
  memset(E.val, 0, total * sizeof(float));
  memset(E.col, 0, total * sizeof(int));

  for (int i = 0; i < A.n; i++) {
    int start = A.row_ptr[i], end = A.row_ptr[i + 1];
    int k = 0;
    for (int j = start; j < end; j++, k++) {
      E.val[i * max_len + k] = A.val[j];
      E.col[i * max_len + k] = A.col_idx[j];
    }
  }
...
// ELLPACK SpMV
void spmv_ell(const ELL &E, const float *x, float *y) {
  int n = E.n, m = E.max_nnz;
  const float *vl = E.val;
  const int *ci = E.col;
  ...
  for (int i = 0; i < n; i++) {
    ...
    int base = i * m;
    for (int j = 0; j < m; j++)
      sum += vl[base + j] * x[ci[base + j]];
    y[i] = sum;
  }
}

// ELLPACK SpMV OpenMP
void spmv_ell_omp(const ELL &E, const float *x, float *y) {
  int n = E.n, m = E.max_nnz;
  const float *vl = E.val;
  const int *ci = E.col;
